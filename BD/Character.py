...
class Character:
    def __init__(self, name, level, online, world, guild,vocation,ultimaMorte,ultimaMorteMobOuPlayer,ultimaMorteNotificada,con):
        self.con=con
    ...
    def insertPersonagem(name,con):
        ...
        if (len(resultSelect) == 0):
            ...
            if (char is None):
                ...
                addCharBd = Character(char.name, char.level, 0, char.world, char.guild_name, char.vocation.name,"0", 0, 1,con)
    ...
    def selectPorID(id, con):
        sqlSelect = "SELECT Character.id,Character.nome,level,online,Vocations.nome FROM Character inner join world on(character.worldid=world.id) inner join Guild on Character.guildid=Guild.id   inner join vocations on (character.vocationid=vocations.id)	where Character.id={}".format(id)
        return con.select(sqlSelect)
    ...
    def selectAllFromGuild(con,guild):
        sqlSelect = "SELECT Character.id,Character.nome,level,online,Vocations.nome,Guild.nome,World.nome,Character.ultimaMorte ,Character.ultimaMorteMobOuPlayer ,Character.ultimaMorteNotificada FROM Character inner join Guild on Character.guildid=Guild.id inner join World on Guild.worldId=World.id inner join Vocations on vocationId = Vocations.id where Guild.nome ILIKE E'{}'".format(guild)
        return con.select(sqlSelect)
    ...
    def selectTotalPlayersGuild(con,guild):
        sqlSelect = "SELECT count(Character.id) FROM Character inner join Guild on Character.guildid=Guild.id  where Guild.nome ILIKE E'{}'".format(guild)
        return con.select(sqlSelect)

    def insert(self):
        result = self.select(self.name,self.con)
        if (len(result)==0):
            sqlinsert = "INSERT INTO Character (nome, level,online,vocationId,guildId,worldId,ultimaMorte,ultimaMorteMobOuPlayer,ultimaMorteNotificada) VALUES(E'{}',{},{},{},{},{},'{}',{},{})".format(self.name, self.level, self.online, self.vocation, self.guild, self.world, self.ultimaMorte,self.ultimaMorteMobOuPlayer, self.ultimaMorteNotificada)
            self.id=self.con.insert(sqlinsert)
            return self.id
        else:
            self.id =result[0][0]
            sqlUpdate = "UPDATE Character SET level = {} , online = {} ,vocationId = {}, guildId = {}, worldId = {} WHERE id = {}".format(self.level, self.online, self.vocation, self.guild, self.world,self.id)
    ...
    @staticmethod
    def updatePorPlayer(name,level,online,vocation,guild,world,con,ultimaMorte,ultimaMorteMobOuPlayer,ultimaMorteNotificada):
            if(online):
                online=1
            else:
                online=0
